...
if __name__ == "__main__":
    ...
    for crit in [skm.meanDistortion, skm.maxDistortion, skm.stdDistortion]:
        proj = skm.Albers.optimize(ra, dec, crit)
        map = skm.Map(proj)
        map.grid()
        map.labelMeridiansAtParallel(0, meridians=[])
        map.footprint(survey, nside=64, zorder=20, facecolor='k', alpha=0.5)
        a,b = proj.distortion(ra, dec)
        c = map.extrapolate(ra, dec, 1-np.abs(b/a), cmap='coolwarm', vmin=0, vmax=0.3, resolution=72)
